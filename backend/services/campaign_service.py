...
class CampaignService:
                ...
                if campaign_type == 'simple_discount':
                    ...
                    if not product_ids or product_id in product_ids:
                        ...
                        if campaign['id'] not in [c['id'] for c in applied_campaigns]:
                            applied_campaigns.append({'id': campaign['id'], 'name': campaign['name'], 'type': 'simple_discount', 'discount': discount})
                    ...
                    if quantity >= min_qty and gift_product_id:
                        ...
                        if gift_product:
                            gift_items.append({'product_id': gift_product_id, 'product_name': gift_product.get('name'), 'quantity': gift_qty, 'unit_price': 0, 'total': 0, 'is_gift': True, 'campaign_name': campaign['name']})
                            if campaign['id'] not in [c['id'] for c in applied_campaigns]:
                                applied_campaigns.append({'id': campaign['id'], 'name': campaign['name'], 'type': 'buy_x_get_y', 'gift_product': gift_product.get('name'), 'gift_quantity': gift_qty})
                    ...
                    if quantity >= bulk_min:
                        ...
                        item_discount += discount
                        if campaign['id'] not in [c['id'] for c in applied_campaigns]:
                            applied_campaigns.append({'id': campaign['id'], 'name': campaign['name'], 'type': 'bulk_discount', 'discount': discount, 'per_unit_discount': discount_per_unit})

            updated_items.append({**item, 'original_price': unit_price, 'discount': item_discount, 'final_price': unit_price - (item_discount / quantity) if quantity > 0 else unit_price, 'total': item_total - item_discount})
            total_discount += item_discount

        return {'items': updated_items, 'gifts': gift_items, 'total_discount': round(total_discount, 2), 'applied_campaigns': applied_campaigns}
    ...
    async def get_campaign_summary(campaign_id: str) -> Dict:
        ...
        if not campaign:
            return None
        campaign_type = campaign.get('campaign_type', 'simple_discount')
        summary = {'id': campaign['id'], 'name': campaign['name'], 'type': campaign_type, 'description': campaign.get('description', ''), 'start_date': campaign['start_date'], 'end_date': campaign['end_date'], 'is_active': campaign['is_active']}
        if campaign_type == 'simple_discount':
